...
async function main() {
  ...
  const transferEvent = receipt.logs.find(
    log => log.topics.length === 4 && log.topics[0] === trackNFT.interface.getEventTopic("Transfer")
  );
  ...
  if (transferEvent) {
    ...
    const sessionEvent = sessionReceipt.logs.find(
      log => log.topics.length > 0 && log.topics[0] === musicSession.interface.getEventTopic("SessionCreated")
    );
    ...
    const joinTx = await musicSession.connect(user2).joinAndCommit(
      sessionId,
      tokenId,
      0 // Drum
    );
